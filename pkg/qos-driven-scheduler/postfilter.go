...
func (scheduler *QosDrivenScheduler) PostFilter(ctx context.Context, state *framework.CycleState, pod *core.Pod, m framework.NodeToStatusMap) (*framework.PostFilterResult, *framework.Status) {
	...
	return &framework.PostFilterResult{NominatedNodeName: nominatedNode}, framework.NewStatus(framework.Success)
}
...
func (scheduler *QosDrivenScheduler) preempt(ctx context.Context, state *framework.CycleState, pod *core.Pod, m framework.NodeToStatusMap) (string, error) {
	...
	if p := scheduler.getUpdatedVersion(pod).LastStatus; p != nil {
		pod = p
	} else {
		klog.Warning("Failed to get updated %v from cache.", pod)
	...
	if len(allNodes) == 0 {
		return "", schedulercore.ErrNoNodesAvailable
	}
	...
	nodeNameToVictims, err := scheduler.selectNodesForPreemption(ctx, fh.PreemptHandle(), state, pod, potentialNodes, pdbs)
	if err != nil {
		return "", err
	}
	...
	if len(candidateNode) == 0 {
		klog.V(1).Infof("[PREEMPT CALL] No node was chosen to allocate pod %s", pod.Name)
		return "", nil
	}
	klog.V(1).Infof("[PREEMPT CALL] The node %v  was chosen to allocate pod %s", candidateNode, pod.Name)
	...
	nominatedPods := scheduler.getLowerPrecedenceNominatedPods(fh.PreemptHandle(), pod, candidateNode)
...
func processPreemptionWithExtenders(fh framework.FrameworkHandle, pod *core.Pod, nodeNameToVictims map[string]*extenderv1.Victims) (map[string]*extenderv1.Victims, error) {
...
func (scheduler *QosDrivenScheduler) selectVictimsOnNode(
	...
	if fits, _, err := schedulercore.PodPassesFiltersOnNode(ctx, ph, state, pod, nodeInfo); !fits {
		if err != nil {
	...
	reprievePod := func(p *core.Pod) (bool, error) {
		...
		fits, _, _ := schedulercore.PodPassesFiltersOnNode(ctx, ph, state, pod, nodeInfo)
		if !fits {
...
func (scheduler *QosDrivenScheduler) getLowerPrecedenceNominatedPods(pn framework.PodNominator, pod *core.Pod, nodeName string) []*core.Pod {
	...
	for _, p := range pods {
		...
		if scheduler.HigherPrecedence(pod, p) {
			lowerPrecedencePods = append(lowerPrecedencePods, p)
		}
	}
