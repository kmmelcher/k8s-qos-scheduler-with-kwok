...
func main() {
	...
	if (len(argsWithoutProg) != 6) {
		fmt.Printf("Invalid number of arguments.\n The expected arguments are <workload_file> <experiment_duration> <service_attack_rate> <number_to_factorial> <interval_period> <number_of_intervals>\n", )
		os.Exit(1)
	}
	...
	interval, _ := strconv.Atoi(argsWithoutProg[4]) /// to be informed in seconds
	fmt.Printf("Interval of web requests submission: %d\n", interval)

	numberOfIntervals, _ := strconv.Atoi(argsWithoutProg[5])
	fmt.Printf("Number of intervals of web requests submission: %d\n", numberOfIntervals)
		...
		for {
			...
			if string(record[0]) != "jid" {
				...
				if controllerKind == "deployment" {
					...
					wg.Add(1)
					go CreateAndManageDemand(controllerName, nodePort, serviceAttackRate, replicaOrParallelismInt, min(experimentDuration-timeRef, deployDuration), factorialNumber, interval, numberOfIntervals, &wg)
...
func CreateAndManageDemand(controllerName string, nodePort int, frequency int, numberOfReplicas int, expectedRuntime int, factorialNumber int, interval int, numberOfIntervals int, wg *sync.WaitGroup) {

	var wgDump sync.WaitGroup

	fmt.Printf("Creating the demmand to service on port %d, which expectedRuntime is %d).\n", nodePort, expectedRuntime)

	controllerFreq := frequency * numberOfReplicas

	for index := 0; index < numberOfIntervals; index++ {
		fmt.Printf("(Interval %d) Waiting to start submission on port %d.\n", index, nodePort)
		// waits 2 minutes
		time.Sleep(time.Duration(120) * time.Second)

		duration :=  time.Duration(interval - 120) * time.Second

		wgDump.Add(1)
		go SubmitAttack(controllerName, nodePort, controllerFreq, duration, factorialNumber, index, &wgDump)

		// waiting interval to start next submission interval
		time.Sleep(time.Duration(interval - 120) * time.Second)
	}

	// wait the intervals latencies before conclude
	wgDump.Wait()
...
func SubmitAttack(controllerName string, nodePort int, freq int, duration time.Duration, factorialNumber int, intervalIndex int, wg *sync.WaitGroup) {

	fmt.Printf("(Interval %d) Submitting web-requests to service on port %d during %d seconds.\n", intervalIndex, nodePort, duration.Seconds())
	...
	fmt.Printf("(Interval %d) Finishing attack on port: %s. Dumping the latencies... \n", intervalIndex, strconv.Itoa(nodePort))

	for _, value := range latencies {
		out += fmt.Sprintf("%f\n", value)
	}
	dump(out, os.Getenv("HOME") + "/" + controllerName + ".interval" + strconv.Itoa(intervalIndex) + ".latency")
	//	dump(out, os.Getenv("HOME") + "/" + controllerName + ".new.latency")

	fmt.Printf("Service running on port: %s\n", strconv.Itoa(nodePort))
	fmt.Printf("Interval Index: %d\n", intervalIndex)
	fmt.Printf("Mean: %s\n", metrics.Latencies.Mean)
	fmt.Printf("50th percentile: %s\n", metrics.Latencies.P50)
	fmt.Printf("95th percentile: %s\n", metrics.Latencies.P95)
	fmt.Printf("99th percentile: %s\n", metrics.Latencies.P99)
	...
	out = fmt.Sprintf("%s,%d,%f,%f,%f,%f,%f,%v,%f,%f\n", controllerName, intervalIndex, metrics.Latencies.Mean.Seconds(),
		metrics.Latencies.P50.Seconds(), metrics.Latencies.P95.Seconds(), metrics.Latencies.P99.Seconds(),
		metrics.Latencies.Max.Seconds(), metrics.Requests, metrics.Success, metrics.Throughput)

	fmt.Printf("(Interval %d) Service running on port: %s - %s\n", intervalIndex, strconv.Itoa(nodePort), out)
...
func getDeploymentSpec(controllerRefName string,
	...
	podTemplate := apiv1.PodTemplateSpec{
		...
		Spec: apiv1.PodSpec{
			...
			Containers: []apiv1.Container{{Name: controllerRefName,
